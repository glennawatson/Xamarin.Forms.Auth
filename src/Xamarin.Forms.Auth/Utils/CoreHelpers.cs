...
{
    [SuppressMessage("Design", "CA1307: Replace should use locality", Justification = "Replace not consistent on all platforms.")]
            ...
            RequestContext requestContext)
        {
            ...
            foreach (string queryPair in queryPairs)
            {
                ...
                if (pair.Count == 2 && !string.IsNullOrWhiteSpace(pair[0]) && !string.IsNullOrWhiteSpace(pair[1]))
                {
                    ...
                    {
                        requestContext?.Logger.Warning(string.Format(
                            CultureInfo.InvariantCulture,
                            "Key/value pair list contains redundant key '{0}'.",
                            key));
                    }
